...
let Notification = (function(vue, globalOption = {
  ...
  let taskColumnLayer = vue.extend(require('./taskColumn.vue'));
  let taskLayer = vue.extend(require('./task.vue'));
  let self = {};
  const defOptions = {
  ...
  self.open = function(options) {
    ...
    if (options.type == 2) {
        ...
        if (!task || task.length === 0) {
            let taskColumnInstance = new taskColumnLayer({
                data: options
            });
            taskColumnInstance.vm = taskColumnInstance.$mount();
            document.body.appendChild(taskColumnInstance.vm.$el);
            // task = document.getElementsByClassName("vl-notify-task");
        }
