        ...
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
          ...
          <el-table-column label="课时ID">
            <template slot-scope="scope">
              <span>{{scope.row.periodId}}</span>
            </template>
          </el-table-column>
          <el-table-column label="评价者ID">
            <template slot-scope="scope">
              <span>{{scope.row.operatorId}}</span>
            </template>
          </el-table-column>
          <el-table-column label="类型 1教练评价, 2学员评价">
            <template slot-scope="scope">
              <span>{{scope.row.type}}</span>
            </template>
          </el-table-column>
          <el-table-column label="教练评价学员名字">
            <template slot-scope="scope">
...
<script>
  import { fetchList, getObj } from '@/api/vehicleevaluate'
  ...
  export default {
    ...
    methods: {
      ...
      create() {
        this.vehicleevaluate = {}
        this.showModule = 'info'
      },
      update(row) {
        getObj(row.roleId)
          .then(response => {
            this.vehicleevaluate = response.data
            this.showModule = 'info'
          })
      },
      search() {
