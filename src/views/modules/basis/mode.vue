    ...
    <el-card style="height: 100%">
      <el-table :key='tableKey' :data="list" v-loading="listLoading" :height="tableHeight-125"  element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button v-if="permissions.basis_mode_type_update" size="small" type="success"
                       @click="handleUpdate(scope.row)">编辑
            </el-button>
            <el-button v-if="permissions.basis_mode_type_del" size="small" type="danger"
      ...
      <div v-show="!listLoading" class="pagination-container" style="margin-top: 10px">
        ...
        <el-button v-if="permissions.basis_mode_type_add" class="filter-item" style="float: right" @click="createClick" type="primary"><i class="el-icon-plus"></i>添加</el-button>
      </div>
    </el-card>
    ...
    <el-dialog :modal="false" @close="cancel('dict')" :title="textMap[dialogStatus]" width="550px" :visible.sync="dialogFormVisible">
      <el-form label-position="left" :model="dict" :rules="rules" ref="dict" label-width="110px">
        <el-form-item label="收款方式"  prop="label">
          <el-input v-model="dict.label" placeholder="收款方式" ></el-input>
        </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          label: [
            { required: true, message: '收款方式', trigger: ['blur', 'change'] }
          ],
          sort: [
