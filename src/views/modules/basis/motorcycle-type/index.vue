    ...
    <el-card :style="{height: (client.height - 40) + 'px'}">
      <el-table :key='tableKey' :data="list" v-loading="listLoading"  :height="$store.state.app.client.height-125"  element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button v-if="basis_motorcycle_type_update" size="small" type="success"
                       @click="handleUpdate(scope.row)">编辑
            </el-button>
            <el-button v-if="basis_motorcycle_type_del" size="mini" type="danger"
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          sort: [
            { required: true, type: 'number', message: '请填写车型排序等级', trigger: 'blur' }
          ]
        },
        dict: {},
        enrolSite: {},
    ...
    created() {
      this.getList()
      this.basis_motorcycle_type_add = this.permissions['basis_motorcycle_type_add']
      this.basis_motorcycle_type_update = this.permissions['basis_motorcycle_type_update']
      this.basis_motorcycle_type_del = this.permissions['basis_motorcycle_type_del']
    },
    methods: {
      getList() {
