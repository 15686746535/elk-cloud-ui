      ...
      <div :style="{height: ($store.state.app.client.height-225) + 'px'}" style="overflow-y: auto;overflow-x: hidden;border-bottom: 1px solid rgb(235, 239, 245)">
        <div v-if="haveQuestion">
          <div style="clear: both;width: 100%;margin: 10px auto;" v-for="(question, index) in questionList">
            <el-row>
              <el-col :span="2">
                <span style="color: #001528;font-size: 16px;">{{revisitQuestion+1}}、</span>
              </el-col>
              <el-col :span="22">
                ...
                <el-row :gutter="2" style="color: #001528;font-size: 16px;">
                  ...
                  <el-col :span="2">
                    <div style="margin: 0 auto;width: 32px; height: 32px">
                      <el-button style="padding: 9px;" size="mini" type="danger" @click="removeQuestion(revisitQuestion)" icon="el-icon-delete" plain circle></el-button>
                    </div>
                  </el-col>
                  <el-col :span="2">
                    <div style="margin: 0 auto;width: 32px; height: 32px">
                      <el-button v-if="revisitQuestion + 1 === questionList.length" style="padding: 9px;" size="mini" type="success" @click="appendQuestion(revisitQuestion)" icon="el-icon-plus" plain circle></el-button>
                    </div>
                  </el-col>
                </el-row>
  ...
  export default {
    ...
    methods: {
      ...
      appendQuestion(index) {
        this.questionList.splice(revisitQuestion + 1, 0, {
          questionnaireId: this.questionnaireId
        })
      },
      /* 删除题目 */
      removeQuestion(index) {
        if (this.questionList.length !== 1) {
          this.questionList.splice(revisitQuestion, 1)
        }
      },
      /* 保存题目 */
      saveQuestion() {
