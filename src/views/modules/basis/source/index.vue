<template xmlns:v-popover="http://www.w3.org/1999/xhtml">
  <div class="app-container calendar-list-container" :style="{height: $store.state.app.client.height + 'px'}">
    <el-card :style="{height: ($store.state.app.client.height - 40) + 'px'}">
      <el-table :key='tableKey' :data="list" v-loading="listLoading" :style="{height: ($store.state.app.client.height-125) + 'px'}" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button v-if="basis_source_update" size="small" type="success"
                       @click="handleUpdate(scope.row)">编辑
            </el-button>
            <el-button v-if="basis_source_del" size="mini" type="danger"
    ...
    <el-dialog :title="textMap[dialogStatus]" width="30%" :visible.sync="dialogFormVisible">
      <el-form label-position="left" :model="dict" :rules="rules" ref="dict" label-width="100px">
        <el-form-item label="来源渠道"  prop="username">
...
<script>
  ...
  export default {
    ...
    created() {
      this.getList()
      this.basis_source_add = this.permissions['basis_source_add']
      this.basis_source_update = this.permissions['basis_source_update']
      this.basis_source_del = this.permissions['basis_source_del']
    },
    methods: {
      getList() {
