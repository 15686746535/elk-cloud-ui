    ...
    <div v-show="showModule=='list'">
      ...
      <el-card>
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
          ...
          <el-table-column label="任务状态    0：成功    1：失败">
            <template slot-scope="scope">
          ...
          <el-table-column label="耗时(单位：毫秒)">
            <template slot-scope="scope">
...
<script>
  import { fetchList, getObj } from '@/api/quartz/schedulejoblog'
  import waves from '@/directive/waves/index.js' // 水波纹指令
  import { removeAllSpace } from '@/utils/validate'

  export default {
    name: 'table_schedulejoblog',
    directives: {
      waves
    },
    data() {
    ...
    methods: {
      ...
      update(row) {
        getObj(row.roleId)
          .then(response => {
            this.schedulejoblog = response.data
            this.showModule = 'info'
          })
      },
      searchClick() {
