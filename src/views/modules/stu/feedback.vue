...
<div>    
  ...
  <el-col :xs="6" :sm="6" :md="6" :lg="4" :xl="4" style="margin-bottom:10px;">
            <el-input @keyup.enter.native="searchClick"  placeholder="姓名/电话/身份证"
                      v-model="queryList.condition"></el-input>
          </el-col>
  ...
  <el-card style="margin-top:10px;">
    <el-table :data="list" empty-text="暂无数据" v-loading="listLoading" :height="pageHeight - 230"  :stripe="true"              
      ...
      <el-table-column align="center"  label="反馈内容" width="150">
        <template slot-scope="scope">
          <span>{{scope.row.feedbackContent}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center"  label="反馈类型" width="auto">
        <template slot-scope="scope">
          <span>{{scope.row.feedbackType}}</span>
        </template>
      </el-table-column>
      
      <el-table-column align="center"  label="反馈结果" width="auto" show-overflow-tooltip>
        <template slot-scope="scope">          
          <!-- 0未处理 1已处理 -->
