    ...
    <div v-show="showModule=='list'">
      ...
      <el-card>
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
          ...
          <el-table-column label="结业时间">
            <template slot-scope="scope">
              <span>{{scope.row.stageTime}}</span>
            </template>
          </el-table-column>
...
<script>
  import { fetchList, getObj } from '@/api/stagegraduation'
  ...
  export default {
    ...
    methods: {
      ...
      create() {
        this.stagegraduation = {}
        this.showModule = 'info'
      },
      update(row) {
        getObj(row.roleId)
          .then(response => {
            this.stagegraduation = response.data
            this.showModule = 'info'
          })
      },
      search() {
